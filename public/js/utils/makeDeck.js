import { NUMBERED_TILES, SUIT_LIST, DRAGON_TILES, DRAGON_SUITS, WIND_TILES, WIND_SUITS, ANIMAL_TILES, ANIMAL_SUITS, FLOWER_TILES, FLOWER_SUITS, WIN_COMBO } from '../rooms/game/tileset.js'
...
const buildTiles = () => {
      ...
      for(let copy = 0; copy < 4; copy++) {
        for (let count = 0; count < 9; count++) {
          let tile = {
            ...
            url : `/public/assets/svgtiles/${NUMBERED_TILES[tiletype].substr(0,1)}${count+1}.svg`,
    ...
    for(let tiletype = 0; tiletype < WIND_TILES.length; tiletype ++) {
      for(let copy = 0; copy < 4; copy++){
        let tile = {
          ...
          url : `/public/assets/svgtiles/${WIND_TILES[tiletype]}.svg`,
  ...
  const insertDragonTiles = () => {
    for(let tiletype = 0; tiletype < DRAGON_TILES.length; tiletype ++) {
      for(let copy = 0; copy < 4; copy++) {
        ...
        tile.url = `/public/assets/svgtiles/dragon_${DRAGON_TILES[tiletype]}.svg`
        tile.copy = copy + 1;
  ...
  const insertFlowerTiles = () => {
    for(let tiletype = 0; tiletype < FLOWER_TILES.length; tiletype ++) {
      ...
      tile.url = `/public/assets/svgtiles/${FLOWER_TILES[tiletype]}.svg`
  ...
  const insertAnimalTiles = () => {
    for(let tiletype = 0; tiletype < ANIMAL_TILES.length; tiletype ++) {
      ...
      tile.url = `/public/assets/svgtiles/${ANIMAL_TILES[tiletype]}.svg`
